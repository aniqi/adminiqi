	...
	*/
	...
	function showTooltip(element, type=0) {
		...
		var height = element.height();
  		...
  		if ( element.is('[type="number"]') ) var width =  $(this).width()+50;


		//+" "+height+" "+width
		element.after("<div class='tooltip'>"+element.attr("tooltip")+"</div>");
		var tooltip = element.parent().find("div.tooltip");
		tooltip.css({top: (top) , left: width+50 , position:'absolute'});
		if (type==1) element.next('.tooltip').addClass('t-green');

		if (type==2) element.next('.tooltip').addClass('t-red');
